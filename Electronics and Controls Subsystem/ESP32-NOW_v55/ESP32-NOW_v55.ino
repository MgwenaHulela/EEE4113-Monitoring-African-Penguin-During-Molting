...
// Generates a filename based on the current timestamp for SD card logging.
String getFilenameFromTimestamp() {
  DateTime now = rtc.now();
  char filename[30]; // /YYYY-MM-DD_HH-MM-SS.txt\0 (26 chars + null terminator)
...
// Writes a line of data to the SD card.
bool writeLog(const String &line) {
  ...
  if (!f) {
    Serial.println("❌ ERROR: Log file open failed for writing.");
  ...
  f.println(line);
  f.close();
  Serial.printf("📁 Logged data to %s\n", filename.c_str());
